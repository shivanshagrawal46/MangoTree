...
if (-not $SkipOriginals -and (Test-Path $Source)) {
  ...
  if ($todo.Count -gt 0) {
    ...
    foreach ($g in $shards) {
      ...
      & ssh @SSHOPTS $Server "mkdir -p '$Dest/raw_store/$shard'"
      $ok = $false
      for ($try = 1; $try -le 3 -and -not $ok; $try++) {
        & scp -q @SSHOPTS -r "$tmp\$shard\*" "${Server}:$Dest/raw_store/$shard/"
        if ($LASTEXITCODE -eq 0) { $ok = $true } else { Write-Host ("      transfer of shard {0} failed (attempt {1}/3), retrying..." -f $shard, $try) -ForegroundColor Yellow; Start-Sleep 5 }
      }
